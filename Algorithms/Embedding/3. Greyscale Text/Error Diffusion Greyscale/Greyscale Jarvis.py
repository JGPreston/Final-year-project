...
def halftoneValue(theGroup):
	for i in range(0, len(theGroup)):
		...
		for ypos, xpos, weighting in jarvisMap:	
...
for file in fileList:									#For every file in the sorted file list
	...
	original = Image.open("../../../Images/Basic Halftone/Error Diffusion/Jarvis/"+filename)		#For comparing against original
	print(filename)
	...
	imageConverted.save("../../../Images/Embedded/3. Greyscale Text/Error Diffusion/Jarvis/"+filename)


	imageDecode = Image.open("../../../Images/Embedded/3. Greyscale Text/Error Diffusion/Jarvis/"+filename)
	start_time = time.time()
