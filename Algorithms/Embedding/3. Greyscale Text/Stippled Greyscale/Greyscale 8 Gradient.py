...
def halftoneValue(theGroup,x,y):
	for i in range(0,len(theGroup)):
		...
		if((255/8)*0 <= theGroup[i] <= (255/8)*1):
			theGroup[i] = gradient0[x][y]
		elif((255/8)*1  <= theGroup[i] <= (255/8)*2):
			theGroup[i] = gradient1[x][y]
		elif((255/8)*2 <= theGroup[i] <= (255/8)*3):
			theGroup[i] = gradient2[x][y]
		elif((255/8)*3 <= theGroup[i] <= (255/8)*4):
			theGroup[i] = gradient3[x][y]
		elif((255/8)*4 <= theGroup[i] <= (255/8)*5):
			theGroup[i] = gradient4[x][y]
		elif((255/8)*5 <= theGroup[i] <= (255/8)*6):
			theGroup[i] = gradient5[x][y]
		elif((255/8)*6 <= theGroup[i] <= (255/8)*7):
			theGroup[i] = gradient6[x][y]
		elif((255/8)*7 <= theGroup[i] <= (255/8)*8):
...
for file in fileList:
	...
	if filename.endswith(".png") or filename.endswith(".jpg") or filename.endswith(".gif"): 
		...
		imageConverted.save("../../../Images/Embedded/3. Greyscale Text/Stippled/8 Gradient/"+filename)


		imageDecode = Image.open("../../../Images/Embedded/3. Greyscale Text/Stippled/8 Gradient/"+filename)
		start_time = time.time()
