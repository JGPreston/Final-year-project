...
def halftoneValue(theGroup,x,y):
	for i in range(0,len(theGroup)):
		if(0 <= theGroup[i] <= (255/2)*1):
			theGroup[i] = gradient1[x][y]
		elif((255/2)*1  <= theGroup[i] <= 255):
...
for file in fileList:
	...
	if filename.endswith(".png") or filename.endswith(".jpg") or filename.endswith(".gif"): 
		...
		imageConverted.save("../../../Images/Embedded/3. Greyscale Text/Stippled/2 Gradient/"+filename)


		imageDecode = Image.open("../../../Images/Embedded/3. Greyscale Text/Stippled/2 Gradient/"+filename)
		start_time = time.time()
