...
def halftoneValue(theGroup,x,y):
	for i in range(0,len(theGroup)):
		...
		if(0 <= theGroup[i] <= (255/6)*1):
			theGroup[i] = gradient0[x][y]
		elif((255/6)*1  <= theGroup[i] <= (255/6)*2):
			theGroup[i] = gradient1[x][y]
		elif((255/6)*2 <= theGroup[i] <= (255/6)*3):
			theGroup[i] = gradient2[x][y]
		elif((255/6)*3 <= theGroup[i] <= (255/6)*4):
			theGroup[i] = gradient5[x][y]
		elif((255/6)*4 <= theGroup[i] <= (255/6)*5):
			theGroup[i] = gradient6[x][y]
		elif((255/6)*5 <= theGroup[i] <= 255):
			theGroup[i] = gradient7[x][y]
		y+=1
...
def halftoneGroup(x,y,imageArray, length):
	for i in range(0,length):
		...
		if(0 <= imageArray[x][y] <= (255/6)*1):
			imageArray[x][y] = gradient0[x][y]
		elif((255/6)*1  <= imageArray[x][y] <= (255/6)*2):
			imageArray[x][y] = gradient1[x][y]
		elif((255/6)*2 <= imageArray[x][y] <= (255/6)*3):
			imageArray[x][y] = gradient2[x][y]
		elif((255/6)*3 <= imageArray[x][y] <= (255/6)*4):
			imageArray[x][y] = gradient5[x][y]
		elif((255/6)*4 <= imageArray[x][y] <= (255/6)*5):
			imageArray[x][y] = gradient6[x][y]
		elif((255/6)*5 <= imageArray[x][y] <= 255):
...
for file in fileList:
	...
	if filename.endswith(".png") or filename.endswith(".jpg") or filename.endswith(".gif"): 
		...
		imageConverted.save("../../../Images/Embedded/3. Greyscale Text/Stippled/6 Gradient/"+filename)


		imageDecode = Image.open("../../../Images/Embedded/3. Greyscale Text/Stippled/6 Gradient/"+filename)
		start_time = time.time()
