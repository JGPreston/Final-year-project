...
def stipple(theHalf):
	for x in range(len(theHalf)):
		for y in range(len(theHalf[0])):
			...
			if(0 <= theHalf[x,y] <= (255/6)*1):
				theHalf[x,y] = gradient0[x,y]
			elif((255/6)*1  <= theHalf[x,y] <= (255/6)*2):
				theHalf[x,y] = gradient1[x,y]
			elif((255/6)*2 <= theHalf[x,y] <= (255/6)*3):
				theHalf[x,y] = gradient2[x,y]
			elif((255/6)*3 <= theHalf[x,y] <= (255/6)*4):
				theHalf[x,y] = gradient5[x,y]
			elif((255/6)*4 <= theHalf[x,y] <= (255/6)*5):
				theHalf[x,y] = gradient6[x,y]
			elif((255/6)*5 <= theHalf[x,y] <= 255):
...
def tempValue(pixel,x,y):
	...
	elif((255/6)*1  <= pixel <= (255/6)*2):
		return gradient1[x,y]
	elif((255/6)*2 <= pixel <= (255/6)*3):
		return gradient2[x,y]
	elif((255/6)*3 <= pixel <= (255/6)*4):
		return gradient5[x,y]
	elif((255/6)*4 <= pixel <= (255/6)*5):
		return gradient6[x,y]
	elif((255/6)*5 <= pixel <= 255):
...
def decode(filename):
	...
	first = Image.open("../../../Images/Embedded/5. Watermark/Stippled/6 Gradient/"+filename)
	firstArray = np.array(first, 'float64')
	second = Image.open("../../../Images/Embedded/5. Watermark/Stippled/6 Gradient/"+filename).rotate(180)
...
for file in fileList:
	...
	original = Image.open("../../../Images/Basic Halftone/Stippled/6 Gradient/"+filename)
	...
	imageConverted.save("../../../Images/Embedded/5. Watermark/Stippled/6 Gradient/"+filename)

	start_time = time.time()
	decodeXOR = decode(filename)
	decryptTime = time.time() - start_time
	
	decodeXOR.save("../../../Images/Embedded/5. Watermark/Stippled/6 Gradient/XOR/"+filename)
