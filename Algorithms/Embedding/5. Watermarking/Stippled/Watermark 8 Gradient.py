...
def stipple(theHalf):
	for x in range(len(theHalf)):
		for y in range(len(theHalf[0])):
			...
			if(0 <= theHalf[x,y] <= (255/8)*1):
				theHalf[x,y] = gradient0[x,y]
			elif((255/8)*1  <= theHalf[x,y] <= (255/8)*2):
				theHalf[x,y] = gradient1[x,y]
			elif((255/8)*2 <= theHalf[x,y] <= (255/8)*3):
				theHalf[x,y] = gradient2[x,y]
			elif((255/8)*3 <= theHalf[x,y] <= (255/8)*4):
				theHalf[x,y] = gradient3[x,y]
			elif((255/8)*4 <= theHalf[x,y] <= (255/8)*5):
				theHalf[x,y] = gradient4[x,y]
			elif((255/8)*5 <= theHalf[x,y] <= (255/8)*6):
				theHalf[x,y] = gradient5[x,y]
			elif((255/8)*6 <= theHalf[x,y] <= (255/8)*7):
				theHalf[x,y] = gradient6[x,y]
			elif((255/8)*7 <= theHalf[x,y] <= 255):
				theHalf[x,y] = gradient7[x,y]
...
def tempValue(pixel,x,y):
	...
	elif((255/8)*1  <= pixel <= (255/8)*2):
		return gradient1[x,y]
	elif((255/8)*2 <= pixel <= (255/8)*3):
		return gradient2[x,y]
	elif((255/8)*3 <= pixel <= (255/8)*4):
		return gradient3[x,y]
	elif((255/8)*4 <= pixel <= (255/8)*5):
		return gradient4[x,y]
	elif((255/8)*5 <= pixel <= (255/8)*6):
		return gradient5[x,y]
	elif((255/8)*6 <= pixel <= (255/8)*7):
		return gradient6[x,y]
	elif((255/8)*7 <= pixel <= 255):
		return gradient7[x,y]
...
def decode(filename):
	...
	first = Image.open("../../../Images/Embedded/5. Watermark/Stippled/8 Gradient/"+filename)
	firstArray = np.array(first, 'float64')
	second = Image.open("../../../Images/Embedded/5. Watermark/Stippled/8 Gradient/"+filename).rotate(180)
...
for file in fileList:
	...
	original = Image.open("../../../Images/Basic Halftone/Stippled/8 Gradient/"+filename)
	...
	imageConverted.save("../../../Images/Embedded/5. Watermark/Stippled/8 Gradient/"+filename)

	start_time = time.time()
	decodeXOR = decode(filename)
	decryptTime = time.time() - start_time
	
	decodeXOR.save("../../../Images/Embedded/5. Watermark/Stippled/8 Gradient/XOR/"+filename)
